...
import { db, Command, ServerSettings } from '..';
import { decodeProgramEmoji } from '../dbinfo';
import { validTeamId, getTeam, makeTeamEmbed } from '../vex';

const yes = '✅';
const no = '❎';
const emojis = [yes, no];

class UpdatesFilterCommand implements Command {
  async execute(message: Message, args: string): Promise<Message> {
    ...
    try {
      if (!teamId.length) {
        const settings = await db().collection<ServerSettings>('settings').findOne({_id: message.guild.id});
      ...
      if (team) {
        reply = await message.reply(unfilter || `add updates filter for ${teamString}?`, {embed: makeTeamEmbed(team)});
      } else {
...
export default new UpdatesFilterCommand();
