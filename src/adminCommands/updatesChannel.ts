...
import { client, db, prefix, Command } from '..';
import { subscribedChannels } from '../vex';

class UpdatesChannelCommand implements Command {
  async execute(message: Message): Promise<Message> {
    ...
    return message.reply(`updates channel set to ${channel}`);
  }
}

export default new UpdatesChannelCommand();
